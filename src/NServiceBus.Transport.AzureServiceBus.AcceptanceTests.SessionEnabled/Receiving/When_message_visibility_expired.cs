...
// Session-enabled receives hand the pipeline a ProcessSessionMessageEventArgs rather than a
// ProcessMessageEventArgs, which is why this test is duplicated here instead of being shared
// with the non-session AcceptanceTests project.
public class When_message_visibility_expired : NServiceBusAcceptanceTest
{
    ...
    static bool WasMarkedAsSuccessfullyCompleted(ScenarioContext.LogItem l, Context c)
        => l.Message.StartsWith($"Received message with id '{c.NativeMessageId}' from session '{c.SessionId}' was marked as successfully completed");
    ...
    public class MyMessageHandler(Context testContext) : IHandleMessages<MyMessage>
    {
        ...
        {
            var messageEventArgs = context.Extensions.Get<ProcessSessionMessageEventArgs>();
            ...
            testContext.SessionId = serviceBusReceivedMessage.SessionId;
