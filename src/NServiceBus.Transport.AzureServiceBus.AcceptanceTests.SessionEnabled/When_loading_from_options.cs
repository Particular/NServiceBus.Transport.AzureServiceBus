namespace NServiceBus.Transport.AzureServiceBus.AcceptanceTests.SessionEnabled;
...
public class When_loading_from_options : NServiceBusAcceptanceTest
{
    string TopicName;
        ...
        await Scenario.Define<Context>()
            .WithEndpoint<Publisher>(b =>
            {
                b.CustomConfig(c =>
                {
                    ...
                    var serializedOptions = JsonSerializer.Serialize(new TopologyOptions { QueueNameToSubscriptionNameMap = { { Conventions.EndpointNamingConvention(typeof(Publisher)), TopicName } }, PublishedEventToTopicsMap = { { typeof(Event).FullName, TopicName } }, SubscribedEventToTopicsMap = { { typeof(Event).FullName, [TopicName] } }, ThrowIfUnmappedEventTypes = true }, TopologyOptionsSerializationContext.Default.TopologyOptions);
    ...
    [Test]
    public async Task Should_throw_exception_when_using_migration_topology()
    {
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        Assert.That(exception.ToString(), Does.Contain("Migration topology is not supported is not supported in session-enabled endpoints."));
    }
