namespace NServiceBus.Transport.AzureServiceBus.Tests.Unmarshalers;
...
public class CloudEventJsonStructuredUnmarshalerTests
{
    ...
    public void Should_unmarshal_regular_json()
    {
        ...
        UnmarshalledMessage actual = RunUnmarshalTest();
    ...
    {
        var actual = Unmarshaler.IsValidMessage(new MessageToUnmarshal(NativeHeaders, Body));
    ...
    UnmarshalledMessage RunUnmarshalTest()
    {
        ...
        var context = new MessageToUnmarshal(NativeHeaders, fullBody);
        return Unmarshaler.CreateIncomingMessage(context);
    }

    void AssertTypicalFields(UnmarshalledMessage actual)
    {
        Assert.Multiple(() =>
        {
